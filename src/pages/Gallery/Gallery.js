...
const Gallery = () => {
    ...
    useEffect(() => {
        ...
        const query1 = '*[_type == "photos"] | order(_createdAt asc){imgUrl , tags-> {title}}';
    ...
    return (
        <>
            ...
            <h1 className='gallery__header head-text'><span>Gallery</span></h1>
            <div className='gallery_container'>
                <div className='gallery_tags'>
                    <h4 onClick={toogleContentVideo} style={{ background: color === "active" ? "#b20000" : "gray" }}>Photos</h4>
                    <h4 onClick={toogleContentPhoto} style={{ background: color === "not-active" ? "#b20000" : "gray" }}>Videos</h4>
                </div>
                <div className={model ? "model open" : "model"}>
                    <img src={tempImgSrc} alt="" />
                    <AiOutlineCloseCircle color='white' size={40} onClick={() => setModel(false)} />
                </div>
                        ...
                        <div className='gallery-photo'>
                            <div className="app__work-filter">
                                ...
                                <p onClick={() => handleWorkFilter('All')}
                                    className={`app__work-filter-item app__flex p-text ${activeFilter === 'All' ? 'item-active' : ''}`}>All</p>

                            </div>
                            <div className='gallery'>

                                {filterWork?.map((photo, index) => (
                                    <div className='pics' key={index} onClick={() => getImg(urlFor(photo.imgUrl))} >
                                        <img src={urlFor(photo.imgUrl)} alt="" style={{ width: "100%" }} />
